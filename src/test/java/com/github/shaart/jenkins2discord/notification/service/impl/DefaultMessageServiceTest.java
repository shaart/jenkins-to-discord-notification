...
import com.github.shaart.jenkins2discord.notification.test.TestMessageConsts;
...
class DefaultMessageServiceTest {
  ...
  @Test
  @DisplayName("Long message with stacktrace splitted correctly by \\n")
  void sendMessageLongMessageWithStacktrace() {
    MessageDto sampleContent = MessageDto.builder()
        .isIgnored(false)
        .content(TestMessageConsts.LONG_EXCEPTION_MESSAGE_WITHOUT_PREFORMATTING)
        .avatarUrl(null)
        .username(null)
        .build();
    makeReturnMessage(sampleContent);
    ...
    TestMessageConsts.EXPECTED_STACKTRACE_PARTS.forEach(expectedPart -> {
      verify(restTemplate, times(1))
          .postForEntity(
              eq(WEBHOOK_URL),
              argThat(messageWithContent(expectedPart)),
              eq(String.class));
    });
  }
